...
${result.key_tradeoffs.map(t => `**${t.decision}**\n- ✓ Pro: ${t.pros.join(' ')}\n- ✗ Con: ${t.cons.join(' ')}`).join('\n\n')}
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-800 p-4 md:p-8 font-sans selection:bg-blue-100">
      <header className="max-w-7xl mx-auto mb-10 text-center md:text-left">
        <h1 className="text-4xl md:text-5xl font-extrabold text-slate-900 tracking-tight mb-2">
          ProofOfWork <span className="text-blue-600">Engine</span>
        </h1>
      ...
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-8">
        ...
        {/* LEFT COLUMN: Inputs (Modern Rounded Card) */}
        <div className="w-full lg:w-1/3 bg-white p-8 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 h-fit sticky top-8">
          ...
          <label className="block text-sm font-semibold text-slate-700 mb-2">Raw Git Log <span className="text-slate-400 font-normal">(Optional)</span></label>
          <textarea 
            className="w-full h-32 p-4 bg-slate-50 border border-slate-200 rounded-2xl mb-8 font-mono text-xs focus:bg-white focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all resize-none shadow-inner"
        ...
        <div className="w-full lg:w-2/3 flex flex-col gap-4">
          ...
          {result && (
            <div className="flex justify-end gap-3 mb-2 animate-fade-in">
          ...
          {!result && !loading && (
            <div className="h-full min-h-[500px] flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-3xl bg-slate-50/50 text-slate-400 font-medium">
          ...
          {loading && (
            <div className="h-full min-h-[500px] flex items-center justify-center rounded-3xl bg-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100">
          ...
          {result && (
            <div id="export-content" className="space-y-6 animate-fade-in pb-10">
              ...
              <div className="bg-white p-8 md:p-10 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100">
                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-6 flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-blue-500"></span> System Architecture Flow</h3>
              ...
              <div className="bg-white p-8 md:p-10 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100">
                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-6 flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-blue-500"></span> Technical Overview</h3>
                <div className="prose prose-slate prose-headings:font-bold prose-a:text-blue-600 max-w-none text-slate-700 leading-relaxed">
                  <ReactMarkdown>{result.system_architecture_markdown}</ReactMarkdown>
                </div>
              </div>

              <div className="bg-white p-8 md:p-10 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 page-break-inside-avoid">
                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-6 flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-blue-500"></span> Strategic Trade-Offs</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  {result.key_tradeoffs.map((tradeoff, idx) => (
                    <div key={idx} className="p-6 bg-slate-50 border border-slate-100 rounded-2xl transition-all hover:shadow-md hover:border-blue-100 group">
                      ...
                      <div className="space-y-3 text-sm">
                        <div className="flex gap-3 items-start bg-white p-3 rounded-xl shadow-sm">
                          <span className="text-green-500 font-bold mt-0.5 bg-green-50 w-6 h-6 flex items-center justify-center rounded-full shrink-0">✓</span>
                          <span className="text-slate-600 leading-relaxed"><strong className="text-slate-800">Pro:</strong> {tradeoff.pros.join(' ')}</span>
                        </div>
                        <div className="flex gap-3 items-start bg-white p-3 rounded-xl shadow-sm">
                          <span className="text-red-500 font-bold mt-0.5 bg-red-50 w-6 h-6 flex items-center justify-center rounded-full shrink-0">✗</span>
